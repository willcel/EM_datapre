...
100 Format(1E14.6)
    ...
    If (ic==3) Then   ! 单极性方波
        ...
        slope1 = (imax-p1(2+(ns_id-1)*2))/tt1
        slope2 = (imax-imin)/tt2
        slope3 = (imin-p4(2+(ns_id-1)*2))/tt3
    ...
    If (ic==0 .Or. ic==1) Then
        ...
        Do i = 1,nt
            ...
            Call frt(rho, hh,mu,t(i), hz1(i), 2, zplus, zminus, ic, ik, nlayer)
    ...
    Else If (ic==2) Then
        Do i = 1, nt
            ...
            Do ip = 1,kpls-1
                ...
                Call frt(rho, hh, mu, t(i)-(ip-1)*(tt1+tt2), s2, 2, zplus, zminus, 0, ik, nlayer)  ! 脉冲响应
                ...
                Call frt(rho, hh, mu, t(i)-(ip-1)*(tt1+tt2)-tt1, s2, 2, zplus, zminus, 0, ik, nlayer)
            ...
            If (t(i)>(kpls-1)*(tt1+tt2))Then
                ...
                Call frt(rho, hh, mu, t(i)-(kpls-1)*(tt1+tt2), s2, 2, zplus, zminus, 0, ik, nlayer)
                ...
                If (t(i)>(kpls-1)*(tt1+tt2)+tt1)Then   ! second delta
                    ...
                    Call frt(rho, hh, mu, t(i)-(kpls-1)*(tt1+tt2)-tt1, s2, 2, zplus, zminus, 0, ik, nlayer)
    ...
    Else If (ic==3) Then
        Do i = 1, nt
            ...
            Do ip = 1, kpls - 1
                ...
                Call frt(rho, hh, mu, t(i)-(ip-1)*(tt1+tt2+tt3+tt4), s2, 2, zplus, zminus, 1, ik, nlayer)

                ss2 = ss2 + s2*slope1

                ik = ik + 1

                Call frt(rho, hh, mu, t(i)-(ip-1)*(tt1+tt2+tt3+tt4)-tt1, s2, 2, zplus, zminus, 1, ik, nlayer)

                ss2 = ss2 + (-1)*s2*(slope1+slope2)

                Call frt(rho, hh, mu, t(i)-(ip-1)*(tt1+tt2+tt3+tt4)-tt1-tt2, s2, 2, zplus, zminus, 1, ik, nlayer)

                ss2 = ss2 + (-1)*s2*(slope3-slope2)

                Call frt(rho, hh, mu, t(i)-(ip-1)*(tt1+tt2+tt3+tt4)-tt1-tt2-tt3, s2, 2, zplus, zminus, 1, ik, nlayer)
            ...
            If (t(i)>(kpls-1)*(tt1+tt2+tt3+tt4))Then
                ...
                Call frt(rho, hh, mu, t(i)-(kpls-1)*(tt1+tt2+tt3+tt4), s2, 2, zplus, zminus, 1, ik, nlayer)
                ...
                If (t(i)>(kpls-1)*(tt1+tt2+tt3+tt4)+tt1)Then
                    ...
                    Call frt(rho, hh, mu, t(i)-(kpls-1)*(tt1+tt2+tt3+tt4)-tt1, s2, 2, zplus, zminus, 1, ik, nlayer)
                    ...
                    If (t(i)>(kpls-1)*(tt1+tt2+tt3+tt4)+tt1+tt2)Then
                        ...
                        Call frt(rho, hh, mu, t(i)-(kpls-1)*(tt1+tt2+tt3+tt4)-tt1-tt2, s2, 2, zplus, zminus, 1, ik, nlayer)

                        ss2 = ss2 + (-1)*s2*(slope3-slope2)

                        If (t(i)>(kpls-1)*(tt1+tt2+tt3+tt4)+tt1+tt2+tt3)Then

                            Call frt(rho, hh, mu, t(i)-(kpls-1)*(tt1+tt2+tt3+tt4)-tt1-tt2-tt3, s2, 2, zplus, zminus, 1, ik, nlayer)
    ...
    Else If (ic==4) Then
     Do i = 1, nt
        ...
        ik = 0
        Do ip = 1, npls
          If (t(i)>(ip-1)*(2.D0*tt1+tt2+tt3) .And. t(i)<ip*(2.D0*tt1+tt2+tt3)) kpls = ip
        End Do

        Do ip = 1, kpls - 1
          Call frt(rho, hh,mu,t(i)-(ip-1)*(2.D0*tt1+tt2+tt3), s2, 2, zplus, zminus, 1, ik, nlayer)

          ss2 = ss2 + (-1)**(ip-1)*s2/tt1

          ik = ik + 1

          Call frt(rho, hh,mu,t(i)-(ip-1)*(2.D0*tt1+tt2+tt3)-tt1, s2, 2, zplus, zminus, 1, ik, nlayer)

          ss2 = ss2 + (-1)**ip*s2/tt1

          Call frt(rho, hh,mu,t(i)-(ip-1)*(2.D0*tt1+tt2+tt3)-tt1-tt2, s2, 2, zplus, zminus, 1, ik, nlayer)

          ss2 = ss2 + (-1)**ip*s2/tt1

          Call frt(rho, hh,mu,t(i)-(ip-1)*(2.D0*tt1+tt2+tt3)-2.D0*tt1-tt2, s2, 2, zplus, zminus, 1, ik, nlayer)

          ss2 = ss2 + (-1)**(ip-1)*s2/tt1

        End Do
!****************** contribution from resting pulses**************************
        If (t(i)>(kpls-1)*(2.D0*tt1+tt2+tt3))Then

          Call frt(rho, hh,mu,t(i)-(kpls-1)*(2.D0*tt1+tt2+tt3), s2, 2, zplus, zminus, 1, ik, nlayer)

          ss2 = ss2 + (-1)**(kpls-1)*s2/tt1

          ik = ik + 1

          If (t(i)>(kpls-1)*(2.D0*tt1+tt2+tt3)+tt1)Then
            Call frt(rho, hh,mu,t(i)-(kpls-1)*(2.D0*tt1+tt2+tt3)-tt1, s2, 2, zplus, zminus, 1, ik, nlayer)

            ss2 = ss2 + (-1)**kpls*s2/tt1

            If (t(i)>(kpls-1)*(2.D0*tt1+tt2+tt3)+tt1+tt2)Then

              Call frt(rho, hh,mu,t(i)-(kpls-1)*(2.D0*tt1+tt2+tt3)-tt1-tt2, s2, 2, zplus, zminus, 1, ik, nlayer)

              ss2 = ss2 + (-1)**kpls*s2/tt1

              If (t(i)>(kpls-1)*(2.D0*tt1+tt2+tt3)+2.D0*tt1+tt2)Then

                Call frt(rho, hh,mu,t(i)-(kpls-1)*(2.D0*tt1+tt2+tt3)-2.D0*tt1-tt2, s2, 2, zplus, zminus, 1, ik, nlayer)
...
Subroutine frt(rho1, hh1,mu1, t, ft, item, zplus, zminus, ic, ik, nlayer)
  Complex *16 fun, iomega, func(5, 67)
  Real *8 t, ft, zplus, zminus, pi, q, rho1(nlayer), hh1(nlayer),mu1(nlayer)
  ...
  If (ik==0) Then
    Do i = 1, nfrq
      Call forward(rho1, hh1, frq(i), func(item,i), item, zplus, zminus, nlayer,mu1) ! mu0*H = B
    End Do
  End If
  Do nn = 1, nc
    n = -nc + ncnull + nn
...
Subroutine forward(rho2, hh2,  f, fun, item, zplus, zminus, nlayer,mu2)
  Complex *16 t3, t5, t6, hf, fun
  Real *8 pi, f, zplus, zminus
  Real *8 r, rho2(nlayer), hh2(nlayer),mu2(nlayer)
  Common /para/r
  pi = 3.1415926D0
  If (item==1)Then
    hf = t6(rho2, hh2,f, zminus, nlayer,mu2)/(4.D0*pi)
  Else If (item==2)Then
    hf = -t3(rho2, hh2,f, zminus, nlayer,mu2)/(4.D0*pi)
  Else If (item==3)Then
    hf = (-t3(rho2, hh2,f,zminus, nlayer,mu2)+t5(rho2, hh2,f,zminus, nlayer,mu2)/r)/(4.D0*pi)
    If (zplus<0.D0) hf = -hf
  Else If (item==4)Then
    hf = t5(rho2, hh2,f, zminus, nlayer,mu2)/(4.D0*pi*r)
    If (zplus<0.D0) hf = -hf
  Else If (item==5) Then
    hf = -t6(rho2, hh2,f, zminus, nlayer,mu2)/(4.D0*pi)
    If (zplus<0.D0) hf = -hf
...
Complex *16 Function t3(rho3, hh3, f, z, nlayer, mu3)
  Complex *16 s, s1, b
  Real *8 r, rho3(nlayer), hh3(nlayer), mu3(nlayer)
  ...
  Do nn = 1, nc
    ...
    nnn = ncnull - nc + nu
    u = expc(-(nnn-1)*fac)/r
    s1 = (b(rho3, hh3,f,u, nlayer, mu3)-u)/(b(rho3, hh3,f,u, nlayer, mu3)+u)*expc(-u*z)*u*u
    s = s + s1*h0(mn)
  End Do
  t3 = s/r
  Return
...
Complex *16 Function t5(rho5, hh5, f, z, nlayer, mu5)
  Complex *16 s, s1, b
  Real *8 r, rho5(nlayer), hh5(nlayer), mu5(nlayer)
  Real *8 h0, h1, f, z, u, fac, expc
  Common /para/r
  Common /hankel/nc, ncnull, h0(100), h1(100)
  fac = 0.1D0*dlog(10.D0)
  ...
  Do nn = 1, nc
    ...
    nnn = ncnull - nc + nu
    u = expc(-(nnn-1)*fac)/r
    s1 = (b(rho5, hh5,f,u, nlayer, mu5)-u)/(b(rho5, hh5,f,u, nlayer, mu5)+u)*expc(-u*z)*u
    s = s + s1*h1(mn)
  End Do
  t5 = s/r
  Return
...
Complex *16 Function t6(rho6, hh6, f, z, nlayer, mu6)
  Complex *16 s, s1, b
  Real *8 r, rho6(nlayer), hh6(nlayer), mu6(nlayer)
  Real *8 h0, h1, f, z, u, fac, expc
  Common /para/r
  Common /hankel/nc, ncnull, h0(100), h1(100)
  fac = 0.1D0*dlog(10.D0)
  ...
  Do nn = 1, nc
    ...
    nnn = ncnull - nc + nu
    u = expc(-(nnn-1)*fac)/r
    s1 = (b(rho6, hh6, f,u, nlayer, mu6)-u)/(b(rho6, hh6, f,u, nlayer, mu6)+u)*expc(-u*z)*u*u
    s = s + s1*h1(mn)
  End Do
  t6 = s/r
  Return
...
! input: mu - 磁导率
! nlayer -  no. of layers
Complex *16 Function b(rho4, hh4, f, u, nlayer, mu4)
  Complex *16 alpha, s1, s2
  Real *8 f, u, pi,gam
  Real *8 r, rho4(nlayer), hh4(nlayer), mu4(nlayer)
  real *8 mu0
  Common /para/r
  ...
  b = cdsqrt(u*u+(0.D0,1.D0)*mu4(nlayer)*2*pi*f/rho4(nlayer))  ! alpha nlayer omega = 2*pi*f
  If (nlayer==1) Return
  Do i = nlayer-1, 1, -1
    gam = mu4(i+1)/mu4(i)
    alpha = cdsqrt(u*u+(0.D0,1.D0)*mu4(i)*2*pi*f/rho4(i))
    s1 = (0.D0, 0.D0)
